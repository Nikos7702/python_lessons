...
class Order:
    ...
    def __iadd__(self, menu):
        if isinstance(menu, Menu):
            for category in menu.categories:
                for dish in category.dishes:
                    self.add_item(dish)
        else:
            raise TypeError("Можна додавати лише об'єкти класу Menu")
        return self

    def __str__(self):
        order_info = "Zamowienie:\n"
        for item in self.items:
            order_info += f"{item.name} - {item.description}, Cena: {item.price} zl\n"
        order_info += f"Calkowita kwota: {self.calculate_total()} zl"
        return order_info
...
order += menu  # добавление блюд из меню в заказ

print(order)  # вывод информации о заказе
